...
diagnose() {
    ...
    local system_gid_threshold=200  # AIX system UID: < 200
    ...
    command_executed="while IFS=: read -r user pw uid gid gecos home shell; do ls -ld \"\$home\" 2>/dev/null; done < /etc/passwd" || true
    ...
    if [ -z "$insecure_homedirs" ]; then
        ...
        command_result="[ls -ld outputs for UID>=200 users]${newline}${raw_stat_output}"
    else
