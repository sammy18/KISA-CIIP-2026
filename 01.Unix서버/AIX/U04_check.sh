...
# @Severity    : 상
# @Title       : 패스워드 파일 보호
# @Description : /etc/passwd 비밀번호 필드가 '!'인지 확인 및 /etc/security/passwd 사용 여부
...
diagnose() {
    ...
    # 진단 로직 구현
    # AIX: /etc/passwd second field should be '!'
    # Encrypted passwords stored in /etc/security/passwd
    
    local shadow_file="/etc/security/passwd"
    ...
    # 1) /etc/security/passwd 파일 존재 확인
    if [ -f "$shadow_file" ]; then
        shadow_exists=true
    fi

    # 2) /etc/passwd 파일의 각 사용자 비밀번호 필드(두 번째 필드) 확인
    while IFS=: read -r username password uid uid_number gid gecos home shell; do
        ...
        if [ -n "$password" ] && [ "$password" != "!" ] && [ "$password" != "*" ]; then
             ...
             non_shadow_users="${non_shadow_users}${username}(pw_field:${password:0:1}...), "
        fi
    done < "$passwd_file"
    ...
    if [ -n "$non_shadow_users" ]; then
         command_result="${command_result}${newline}${newline}[Warning: Non-Secure Users]${newline}${non_shadow_users%, }"
    fi
    ...
    if [ "$shadow_exists" = true ] && [ "$all_users_shadow" = true ]; then
        ...
        status="양호"
        inspection_summary="쉐도우 패스워드 사용 및 /etc/passwd 파일 보호 양호"
    else
        diagnosis_result="VULNERABLE"
        status="취약"
        inspection_summary="쉐도우 패스워드 미사용 또는 /etc/passwd 파일에 평문 암호 존재"
    fi
