...
diagnose() {
    ...
    local umask_grep=$(grep -h 'umask' /etc/profile /etc/bash.bashrc /root/.bashrc 2>/dev/null | grep -v "^[[:space:]]*#" || echo "No umask settings found")
    command_result="[Command: grep -h 'umask' /etc/profile /etc/bash.bashrc /root/.bashrc]${newline}${umask_grep}"
    ...
    if [ $config_files_checked -eq 0 ]; then
        ...
        local grep_umask=$(grep -h 'umask' /etc/profile /etc/bash.bashrc /root/.bashrc 2>/dev/null || echo "No UMASK found")
        command_result="[Command: grep -h 'umask' /etc/profile /etc/bash.bashrc /root/.bashrc]${newline}${grep_umask}"
        command_executed="grep -h 'umask' /etc/profile /etc/bash.bashrc /root/.bashrc 2>/dev/null"
    fi
