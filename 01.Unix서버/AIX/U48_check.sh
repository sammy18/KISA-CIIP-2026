...
diagnose() {
    ...
    elif lssrc -s postfix 2>/dev/null | grep -q "active"; then
        smtp_running=true
        smtp_service="postfix"
    fi

    command_executed="lssrc -s sendmail 2>/dev/null; grep 'PrivacyOptions' /etc/mail/sendmail.cf 2>/dev/null"
