...
diagnose() {
    ...
    if [ -f "/etc/hosts.equiv" ]; then
        ...
        local perms=$(ls -ld "/etc/hosts.equiv" 2>/dev/null | awk '{print $1}' | cut -c2-10 | sed 's/rwx/7/g; s/rw-/6/g; s/r-x/5/g; s/r--/4/g; s/-wx/3/g; s/-w-/2/g; s/--x/1/g; s/---/0/g')
        local owner=$(ls -ld "/etc/hosts.equiv" 2>/dev/null | awk '{print $3":"$4}')
    ...
    while IFS= read -r user_line; do
        ...
        if [ -d "$home_dir" ] && [ "$uid" -ge 200 ] 2>/dev/null; then
            ...
            if [ -f "$rhosts_path" ]; then
                ((rhosts_count++)) || true
                local perms=$(ls -ld "$rhosts_path" 2>/dev/null | awk '{print $1}' | cut -c2-10 | sed 's/rwx/7/g; s/rw-/6/g; s/r-x/5/g; s/r--/4/g; s/-wx/3/g; s/-w-/2/g; s/--x/1/g; s/---/0/g')
                local owner=$(ls -ld "$rhosts_path" 2>/dev/null | awk '{print $3}')
    ...
    if [ "$hosts_equiv_exists" -eq 0 ] && [ "$rhosts_count" -eq 0 ]; then
        ...
        local find_rhosts=$(find /home -name '.rhosts' 2>/dev/null | head -10 || echo "No .rhosts files")
