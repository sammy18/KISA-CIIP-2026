...
diagnose() {
    ...
    # 2. 장기 미사용 계정 확인 (참고용 - AIX는 lsuser 등으로 확인 가능하나 여기서 lastlog 시도)
    ...
    if command -v lastlog >/dev/null 2>&1; then
        ...
        if command -v lsuser >/dev/null 2>&1; then
             lastlog_output="[AIX lsuser output (Sample)]${newline}$(lsuser -a time_last_login ALL | head -20)"
        else
             lastlog_output="lastlog/lsuser command not available"
        fi
    fi
