...
diagnose() {
    ...
    # 3) SMF 서비스 확인 (Solaris: svcs 사용)
    service_status="${service_status}\\nSMF 서비스 확인:\\n"
    for svc in "${vulnerable_services[@]}"; do
        if svcs "${svc}" 2>/dev/null | grep -q online; then
            service_status="${service_status}  ${svc}: online\\n"
            is_secure=false
            enabled_services+=("smf: ${svc}")
        else
            service_status="${service_status}  ${svc}: offline 또는 없음\\n"
        fi
    done || true
    ...
    if [ "$is_secure" = true ]; then
        ...
        command_executed="grep -iE '^echo|^chargen|^daytime' /etc/inetd.conf 2>/dev/null; svcs echo chargen daytime 2>/dev/null | grep online; ss -tuln | grep -E ':7|:13|:19'"
    else
        diagnosis_result="VULNERABLE"
        status="취약"
        inspection_summary="DoS 취약 서비스 활성화됨: ${enabled_services[*]}"
        command_result="${service_status}"
        command_executed="grep -iE '^echo|^chargen|^daytime' /etc/inetd.conf 2>/dev/null; svcs echo chargen daytime 2>/dev/null | grep online"
    fi
