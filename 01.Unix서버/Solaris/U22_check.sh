...
diagnose() {
    ...
    if [ ! -f "$target_file" ]; then
        ...
        # 파일 권한 확인 (Solaris: perl stat 사용) - 실제 명령어 결과 저장
        local raw_output=$(ls -ld "$target_file" 2>/dev/null)
        local file_perms=$(perl -e 'if (-f $ARGV[0]) { printf "%04o\n", (stat($ARGV[0]))[2] & 07777; }' "$target_file" 2>/dev/null)
        ...
        if [ "$is_secure" = true ]; then
            ...
            command_result="[Command: ls -ld]${newline}${raw_output}${newline}${newline}[Command: perl stat]${newline}${stat_output}"
            command_executed="ls -ld $target_file; perl -e 'printf \"%04o\n\", (stat)[2] & 07777' $target_file"
        else
