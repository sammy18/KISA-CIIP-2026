...
diagnose() {
    ...
    # 1) tftp 서비스 확인 (Solaris SMF)
    if svcs network/tftp 2>/dev/null | grep -q "online"; then
        services_running=true
        running_services+=("tftp")
        service_info="${service_info}tftp: online${newline}"
    else
        service_info="${service_info}tftp: offline${newline}"
    fi

    # in.tftpd (inetd 기반) 확인
    if [ -f /etc/inetd.conf ]; then
        local tftp_inetd=$(grep "^tftp" /etc/inetd.conf 2>/dev/null | grep -v "^#" || echo "")
    ...
    # 2) talk/ntalk 서비스 확인 (Solaris SMF)
    for svc in talk ntalk; do
        if svcs "network/${svc}" 2>/dev/null | grep -q "online"; then
            services_running=true
            running_services+=("$svc")
            service_info="${service_info}${svc}: online${newline}"
        else
            service_info="${service_info}${svc}: offline${newline}"
        fi
    done || true
    ...
    if command -v netstat &>/dev/null; then
        local tftp_port=$(netstat -an | grep "\.69 " || echo "")
        local talk_port=$(netstat -an | grep -E "\.517 |\.518 " || echo "")
    ...
    if [ "$services_running" = false ]; then
        ...
        local svc_output=$(svcs network/tftp network/talk network/ntalk 2>/dev/null || echo "Services not found")
        command_result="[Command: svcs network/tftp network/talk network/ntalk]${newline}${svc_output}"
        command_executed="svcs network/tftp network/talk network/ntalk; netstat -an | grep -E '\.69|\.517|\.518'"
    else
        diagnosis_result="VULNERABLE"
        status="취약"
        inspection_summary="불필요한 서비스 활성화됨: ${running_services[*]}"
        command_result="${service_info}"
        command_executed="svcs network/tftp network/talk network/ntalk; grep -E '^talk|^ntalk' /etc/inetd.conf"
    fi
