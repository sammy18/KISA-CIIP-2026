...
diagnose() {
    ...
    if [ "$mail_installed" = false ]; then
        ...
        command_result="Mail service: [not used]${newline}${newline}[Sendmail]${newline}${sendmail_cmd_output}${newline}${newline}[Postfix]${newline}${postconf_cmd_output}${newline}${newline}[Exim]${newline}${exim_cmd_output}"
        ...
        command_executed="sendmail -d0.4 -bv < /dev/null; postconf mail_version; exim --version; systemctl is-active sendmail postfix exim4"
    fi
