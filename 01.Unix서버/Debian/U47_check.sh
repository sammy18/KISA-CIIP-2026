...
diagnose() {
    ...
    if [ -f /etc/mail/sendmail.cf ] || [ -f /etc/sendmail.cf ]; then
        ...
        relay_info="${relay_info}Sendmail PrivacyOptions:${newline}${privacy_options}${newline}"

        if echo "$privacy_options" | grep -qi "goaway"; then
            relay_restricted=true
            relay_info="${relay_info}goaway 옵션으로 릴레이 제한됨${newline}"
        fi

        # access.db 파일 확인
        if [ -f /etc/mail/access.db ] || [ -f /etc/mail/access ]; then
            relay_restricted=true
            relay_info="${relay_info}Sendmail access DB 존재${newline}"
        fi
    fi
    ...
    if command -v postconf &>/dev/null; then
        ...
        relay_info="${relay_info}Postfix smtpd_relay_restrictions:${newline}${relay_restrictions}${newline}"

        if echo "$relay_restrictions" | grep -q "permit_mynetworks"; then
            relay_restricted=true
            relay_info="${relay_info}permit_mynetworks로 제한됨${newline}"
        fi

        if echo "$relay_restrictions" | grep -q "reject_unauth_destination"; then
            relay_restricted=true
            relay_info="${relay_info}reject_unauth_destination로 제한됨${newline}"
        fi
        ...
        relay_info="${relay_info}smtpd_recipient_restrictions:${newline}${recipient_restrictions}${newline}"

        # mynetworks 설정 확인
        local mynetworks=$(postconf mynetworks 2>/dev/null | grep "mynetworks" | awk '{print $3}')
        relay_info="${relay_info}mynetworks: ${mynetworks}${newline}"

        # relay_domains 확인
        local relay_domains=$(postconf relay_domains 2>/dev/null | grep "relay_domains" | awk '{print $3}')
        relay_info="${relay_info}relay_domains: ${relay_domains}${newline}"
    fi

    # 3) open relay 테스트 (기본 확인만)
    if command -v nc &>/dev/null; then
        relay_info="${relay_info}open relay 테스트는 수동으로 수행 필요${newline}"
    fi
