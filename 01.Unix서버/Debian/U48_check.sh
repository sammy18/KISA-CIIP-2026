...
diagnose() {
    ...
    # 1) vsftpd 확인
    if systemctl is-active vsftpd &>/dev/null || systemctl is-active pure-ftpd &>/dev/null || systemctl is-active proftpd &>/dev/null; then
        ftp_running=true
        ...
        if [ -f /etc/debian_version ]; then
            local pkg_version=$(dpkg -l | grep -E "vsftpd|proftpd|pure-ftpd" | awk '{print $2, $3}' || echo "")
            if [ -n "$pkg_version" ]; then
                ftp_details="${ftp_details}${newline}패키지: ${pkg_version}"
            fi
        fi
    fi
    ...
    if command -v ss &>/dev/null; then
        ...
        if [ -n "$ftp_port" ]; then
            ftp_running=true
            ftp_details="${ftp_details}${newline}FTP 포트 21 활성화"
        fi
    fi
    ...
    if [ "$ftp_running" = false ]; then
        ...
        command_result="[FTP Service Status]${newline}$(systemctl is-active vsftpd 2>&1 || systemctl is-active pure-ftpd 2>&1 || systemctl is-active proftpd 2>&1 || echo 'FTP services not running')${newline}${newline}[Port 21 Check]${newline}$(ss -tuln 2>/dev/null | grep ':21 ' || echo 'No FTP service listening on port 21')"
        command_executed="systemctl is-active vsftpd pure-ftpd proftpd; ss -tuln | grep ':21 '"
    else
        # 버전 확인이 가능한 경우
        if [ -n "$ftp_version" ] && [ "$ftp_version" != "unknown" ]; then
            ...
            command_executed="vsftpd -version 2>/dev/null; proftpd -v 2>/dev/null; dpkg -l | grep -E 'vsftpd|proftpd'"
        else
            diagnosis_result="MANUAL"
            status="수동진단"
            inspection_summary="FTP 서비스 실행 중, 버전 확인 필요: ${ftp_details}"
            command_result="${ftp_details}"
            command_executed="systemctl is-active vsftpd pure-ftpd proftpd; vsftpd -version 2>/dev/null"
        fi
    fi
