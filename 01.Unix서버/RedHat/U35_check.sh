...
diagnose() {
    ...
    if [ "$ftp_installed" = false ]; then
        ...
        local raw_ftp_check=$(command -v vsftpd proftpd pure-ftpd 2>&1; systemctl is-active vsftpd proftpd pure-ftpd 2>&1)
        command_result="[Command: command -v vsftpd proftpd pure-ftpd; systemctl is-active vsftpd proftpd pure-ftpd]${newline}${raw_ftp_check}"
