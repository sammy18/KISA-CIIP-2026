...
diagnose() {
    ...
    # 진단 로직 구현
    # RedHat/CentOS: /etc/pam.d/su 파일 설정 확인 (pam_wheel.so)
    ...
    if [ -f "$pam_file" ]; then
        ...
        # Check if pam_wheel.so is uncommented and uses 'use_uid' or 'group=wheel'
        # Note: mere existence of authorized line implies restriction
        if [ -n "$pam_output" ]; then
            if echo "$pam_output" | grep -qE "use_uid|group="; then
                ...
                # trust option alone might be insufficient if not accompanied by a restriction line
                 check_type="PAM (Weak)"
                 details="pam_wheel.so trust 옵션 존재 (주의 필요)"
            else
                 # Just pam_wheel.so required usually implies restriction to wheel
                 is_secure=true
                 check_type="PAM"
                 details="pam_wheel.so 설정 확인됨"
            fi
    ...
    # 2. File Permission Assessment (Alternative if PAM not configured or as secondary check)
    # If PAM check passed, we usually consider it GOOD, but checking permissions is informative.
    # If PAM check failed, checking permissions is critical to see if restricted by filesystem.
    
    local su_stat=$(ls -l "$su_bin")
    local su_perm=$(stat -c "%a" "$su_bin" 2>/dev/null || echo "")
    local su_group=$(stat -c "%G" "$su_bin" 2>/dev/null || echo "")
    
    su_perm_output="[File: $su_bin]${newline}${su_stat}"

    if [ "$is_secure" = false ]; then
        # Check if 4750 or 4770 AND group is wheel (or specific admin group)
        # Usually looking for o-rwx (others cannot execute)
        
        # Check others permission (last digit)
    ...
    command_result="[File: $pam_file]${newline}$(grep "pam_wheel.so" "$pam_file" 2>/dev/null || echo "pam_wheel.so configuration not found")${newline}${newline}${su_perm_output}${newline}${newline}${wheel_group_output}"
