...
diagnose() {
    ...
    if systemctl is-active rsyslog >/dev/null 2>&1; then
        ...
        local raw_syslog_check=$(systemctl status rsyslog 2>&1; systemctl status syslog-ng 2>&1)
        command_result="[Command: systemctl status rsyslog; systemctl status syslog-ng]${newline}${raw_syslog_check}"
        command_executed="systemctl status rsyslog; systemctl status syslog-ng"
    ...
    if [ ! -f "$config_file" ]; then
        ...
        local raw_config_check=$(ls -l ${config_file} 2>&1)
        command_result="[Command: ls -l ${config_file}]${newline}${raw_config_check}"
        ...
        command_executed="systemctl status ${syslog_service}; ls -l /var/log/*.log | head -10"
        ...
        if [ "$service_active" = true ] && [ "$config_lines" -gt 0 ] && [ "$log_files_check" -ge 2 ]; then
            ...
            command_result="${syslog_service} Active, Config lines: ${config_lines}, Log files recording: ${log_files_check}"
