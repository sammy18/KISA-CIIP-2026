...
diagnose() {
    ...
    if [ "$snmpd_installed" = false ]; then
        ...
        if [ "$has_acl" = true ]; then
            ...
            status="취약"
            inspection_summary="SNMP 접근 제어가 설정되지 않음 (모든 접근 허용 가능): ${acl_details:-No access control}"
            local raw_acl_check=$(grep -v '^#' ${snmp_conf} 2>/dev/null | grep -E 'com2sec|rouuser|rwuser|agentAddress' || echo "No ACL settings")
            command_result="[Command: grep -v '^#' ${snmp_conf} | grep -E 'com2sec|rouuser|rwuser|agentAddress']${newline}${raw_acl_check}"
