...
diagnose() {
    ...
    local ww_files=""
    local ww_count=0
    local sys_dirs="/ /home /tmp /var /usr /opt /etc /boot"

    # Capture raw find output for world-writable files
    local ww_files_raw=""
    # 시스템 주요 디렉터리에서 world-writable 파일 검색
    for dir in $sys_dirs; do
        if [ -d "$dir" ]; then
            local dir_find=$(find "$dir" -perm -2 -type f 2>/dev/null | head -30)
            if [ -n "$dir_find" ]; then
                ww_files_raw="${ww_files_raw}[Directory: ${dir} (files)]${newline}${dir_find}${newline}"
            fi
    ...
    for dir in $sys_dirs; do
        if [ -d "$dir" ]; then
            local dir_find=$(find "$dir" -perm -2 -type d 2>/dev/null | head -20)
            if [ -n "$dir_find" ]; then
                ww_dirs_raw="${ww_dirs_raw}[Directory: ${dir} (directories)]${newline}${dir_find}${newline}"
            fi
    ...
    if [ "$ww_count" -eq 0 ] && [ "$ww_dir_count" -eq 0 ]; then
        ...
        command_result="[Command: find world-writable files]${newline}${ww_files_raw}${newline}${newline}[Command: find world-writable directories]${newline}${ww_dirs_raw}"
        command_executed="find / /home /tmp /var /usr /opt /etc /boot -perm -2 -type f 2>/dev/null; find / /home /tmp /var /usr /opt /etc /boot -perm -2 -type d 2>/dev/null"
    else
