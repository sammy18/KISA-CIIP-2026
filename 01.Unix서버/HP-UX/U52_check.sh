...
diagnose() {
    ...
    # 1) Telnet 서비스 실행 여부 확인 (HP-UX init.d)
    if [ -f /sbin/init.d/telnetd ] || [ -f /sbin/init.d/telnet ]; then
        if /sbin/init.d/telnetd status 2>/dev/null | grep -q "running" 2>/dev/null || /sbin/init.d/telnet status 2>/dev/null | grep -q "running" 2>/dev/null; then
            telnet_running=true
            telnet_details="init.d 서비스 상태: running"
        fi
    fi
    ...
    if swlist 2>/dev/null | grep -q "telnetd"; then
    ...
    if [ "$telnet_running" = true ]; then
        ...
        if [ -n "$telnet_installed" ]; then
            ...
            local telnet_check=$(swlist 2>/dev/null | grep telnetd; /sbin/init.d/telnetd status 2>/dev/null | head -2 || echo "Telnet not found")
