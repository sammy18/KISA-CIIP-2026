...
diagnose() {
    ...
    if [ "$snmpd_installed" = false ]; then
        ...
        local snmp_not_installed=$(ls /etc/snmp/snmpd.conf 2>/dev/null || echo "SNMP not installed")
        ...
        local no_snmp_conf=$(ls /etc/snmp/*.conf 2>/dev/null || echo "No SNMP config files found")
        command_result="${no_snmp_conf}"
        ...
        if [ "$weak_community" = true ]; then
            ...
            if [ -n "$custom_communities" ]; then
                ...
                local secure_comm=$(grep -iE 'com2sec|rocommunity|rwcommunity' /etc/snmp/snmpd.conf 2>/dev/null | head -5)
                command_result="Community: secure settings\n${secure_comm}"
            else
                inspection_summary="SNMP Community String이 설정되지 않았거나 v3만 사용 중"
                local no_comm=$(grep -iE 'com2sec|rocommunity|rwcommunity' /etc/snmp/snmpd.conf 2>/dev/null | head -3)
