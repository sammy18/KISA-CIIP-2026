...
diagnose() {
    ...
    # 1) syslog 서비스 확인
    if /sbin/init.d/rsyslog status 2>/dev/null | grep -q "running" >/dev/null 2>&1; then
        syslog_service="rsyslog"
        service_active=true
        config_file="/etc/rsyslog.conf"
    elif /sbin/init.d/syslog-ng status 2>/dev/null | grep -q "running" >/dev/null 2>&1; then
        syslog_service="syslog-ng"
        service_active=true
        config_file="/etc/syslog-ng/syslog-ng.conf"
    elif /sbin/init.d/sysklogd status 2>/dev/null | grep -q "running" >/dev/null 2>&1; then
        ...
        local syslog_check=$(/sbin/init.d/rsyslog status 2>/dev/null | head -2; /sbin/init.d/syslog-ng status 2>/dev/null | head -2 || echo "No syslog service found")
        command_result="${syslog_check}"
        command_executed="/sbin/init.d/ status rsyslog; /sbin/init.d/ status syslog-ng"
    ...
    if [ ! -f "$config_file" ]; then
        ...
        command_result="${config_file} [not found]"
