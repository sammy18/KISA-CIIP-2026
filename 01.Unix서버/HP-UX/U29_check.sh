...
diagnose() {
    ...
    # Capture ls and init.d output
    local hosts_lpd_ls=$(ls -l /etc/hosts.lpd 2>&1)
    local lpd_status=$(/sbin/init.d/cups status 2>/dev/null || /sbin/init.d/lpd status 2>/dev/null || echo "No LPD service found")
    command_result="[Command: ls -l /etc/hosts.lpd]${newline}${hosts_lpd_ls}${newline}${newline}[Command: /sbin/init.d/cups/lpd status]${newline}${lpd_status}"
    ...
    # LPD 서비스 실행 여부 확인
    local lpd_running=0
    if command -v lpstat &>/dev/null || /sbin/init.d/cups status 2>/dev/null | grep -q "running" 2>/dev/null || /sbin/init.d/lpd status 2>/dev/null | grep -q "running" 2>/dev/null; then
        ((lpd_running++)) || true
    fi
    ...
    if [ "$hosts_lpd_exists" -eq 0 ]; then
        if [ "$lpd_running" -eq 0 ]; then
            ...
            local lpd_check=$(ls -l /etc/hosts.lpd 2>/dev/null || echo "No hosts.lpd file")
            command_result="${lpd_check}"
            command_executed="ls -l /etc/hosts.lpd 2>/dev/null; /sbin/init.d/ status cups lpd 2>/dev/null | grep -q active"
        else
            # LPD 서비스 사용 중이나 hosts.lpd 없음 (최신 시스템은 CUPS 사용)
            diagnosis_result="GOOD"
            status="양호"
            inspection_summary="LPD/CUPS 서비스 사용 중이나 hosts.lpd 파일 없음 (최신 시스템에서는 정상)"
            local cups_check=$(ls -l /etc/hosts.lpd 2>/dev/null; /sbin/init.d/cups status 2>/dev/null | head -3 || echo "CUPS status unknown")
            command_result="${cups_check}"
            command_executed="ls -l /etc/hosts.lpd 2>/dev/null; /sbin/init.d/ status cups 2>/dev/null"
        fi
    else
        # hosts.lpd 파일 존재
