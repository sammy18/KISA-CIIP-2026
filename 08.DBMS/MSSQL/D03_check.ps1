...
# GUIDELINE 정보
$purpose = '비밀번호 사용기간 및 복잡도 설정 유무를 점검하여 비인가자의 비밀번호 추측 공격(무차별 대입 공격, 사전 대입 공격 등)에 대한 대비가 되어 있는지 확인하기 위함'
$threat = '약한 비밀번호 정책 시 쉽게 유추 가능한 비밀번호 사용으로 인한 무단 접근 위험'
$criteria_good = '비밀번호 정책이 적절히 설정된 경우'
$criteria_bad = '비밀번호 정책이 설정되지 않은 경우'
$remediation = 'Windows Server 보안 정책 또는 SQL Server 정책에서 비밀번호 정책(복잡도, 사용기간, 잠금 임계값 등) 설정'
