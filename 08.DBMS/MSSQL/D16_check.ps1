...
# GUIDELINE 정보
$purpose = 'Windows 인증 모드를 사용하여 혼합 인증 모드의 취약점 방지 및 보안 강화'
$threat = 'SQL Server 및 Windows 인증 모드 사용 시 SA 계정 무력화 공격, Brute Force, Dictionary 공격 등의 위험에 노출'
$criteria_good = 'Windows 인증 모드만 사용하는 경우'
$criteria_bad = 'SQL Server 및 Windows 인증 모드(혼합 모드)를 사용하는 경우'
$remediation = 'SQL Server Management Studio에서 서버 속성 > 보안 > Windows 인증 모드만 선택'
