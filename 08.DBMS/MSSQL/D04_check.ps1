...
# GUIDELINE 정보
$purpose = '관리자 권한이 필요한 계정과 그룹에만 관리자 권한을 부여하였는지 점검하여 관리자 권한의 남용을 방지하여 계정 유출로 인한 비인가자의 DB 접근 가능성을 최소화하고자 함'
$threat = '관리자 권한이 필요 없는 계정 및 그룹에 관리자 권한이 부여된 경우 관리자 권한이 부여된 계정이 비인가자에게 유출될 경우 DB에 접근할 수 있는 위험이 존재함'
