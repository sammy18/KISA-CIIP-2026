...
# GUIDELINE 정보
$purpose = 'Object Owner가 인가된 계정에게만 존재하는지 점검'
$threat = 'Object Owner가 일반 사용자에게 존재하는 경우 공격자가 이를 이용하여 Object의 수정, 삭제가 가능하여 중요정보의 유출 및 변경의 위험이 존재함'
$criteria_good = 'Object Owner가 dbo 또는 sysadmin 권한 계정으로 제한된 경우'
$criteria_bad = 'Object Owner가 일반 사용자에게도 존재하는 경우'
$remediation = 'Object Owner를 dbo 또는 sysadmin 권한 계정으로 변경: ALTER AUTHORIZATION ON OBJECT::schema.table TO dbo;'
