...
# @Description : CREATE 권한을 제어하여 데이터베이스/테이블 무단 생성 방지
...
diagnose() {
    ...
    # CREATE 권한 확인
    local create_query="SELECT user, host FROM mysql.user WHERE Create_priv='Y' ORDER BY user, host;"
    command_executed="mysql -h "${DB_HOST}" -P "${DB_PORT}" -u "${DB_USER}" -p"${DB_PASSWORD}" -e \"${create_query}\""
    command_result=$(mysql -h "${DB_HOST}" -P "${DB_PORT}" -u "${DB_USER}" -p"${DB_PASSWORD}" -e "${create_query}" 2>/dev/null || echo "")

    if [ -z "$command_result" ]; then
        # MySQL 8.0+의 경우
        command_result=$(mysql -h "${DB_HOST}" -P "${DB_PORT}" -u "${DB_USER}" -p"${DB_PASSWORD}" -e "SELECT grantee, table_schema FROM information_schema.role_table_grants WHERE privilege_type='CREATE' LIMIT 20;" 2>/dev/null || echo "")
    fi

    # 결과 분석
    if [ -n "$command_result" ]; then
        local create_count=$(echo "$command_result" | tail -n +2 | grep -v "^$" | wc -l)

        if [ "$create_count" -gt 0 ]; then
            local create_users=$(echo "$command_result" | tail -n +2 | grep -v "^$" || echo "")
            local non_root_create=$(echo "$create_users" | grep -v -E "^root\s" || echo "")

            if [ -n "$non_root_create" ]; then
                diagnosis_result="VULNERABLE"
                status="취약"
                inspection_summary="비-root 계정에 CREATE 권한 부여됨: $(echo "$non_root_create" | head -5 | tr '\n' ', ')"
            else
                diagnosis_result="GOOD"
                status="양호"
                inspection_summary="root 계정에만 CREATE 권한 부여됨 (총 ${create_count}개)"
            fi
        else
            diagnosis_result="GOOD"
            status="양호"
