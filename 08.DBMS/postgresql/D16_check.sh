...
diagnose() {
    ...
    local drop_query="SELECT grantee, table_schema FROM information_schema.role_table_grants WHERE privilege_type='DROP' AND grantee NOT IN ('postgres', 'pg_signal_backend') LIMIT 20;"
    command_executed="psql -h ${DB_HOST} -p ${DB_PORT} -U ${DB_ADMIN_USER} -d postgres -c \"${drop_query}\""

    # Try Unix socket connection first (peer authentication in Docker)
    command_result=$(psql -U "${DB_ADMIN_USER}" -d postgres -c "${drop_query}" 2>/dev/null || echo "")

    # Fall back to TCP connection with password
    if [ -z "$command_result" ]; then
        command_result=$(PGPASSWORD="${DB_ADMIN_PASS}" psql -h "${DB_HOST}" -p "${DB_PORT}" -U "${DB_ADMIN_USER}" -d postgres -c "${drop_query}" 2>/dev/null || echo "")
    fi

    if [ -n "$command_result" ]; then
        local drop_count=$(echo "$command_result" | grep -v "^$" | grep -v "^--" | grep -v "^grantee" | wc -l)

        if [ "$drop_count" -gt 0 ]; then
            local drop_users=$(echo "$command_result" | grep -v "^$" | grep -v "^--" | grep -v "^grantee" || echo "")

            diagnosis_result="VULNERABLE"
            status="취약"
            inspection_summary="DROP 권한을 가진 ${drop_count}개 그랜트 발견: $(echo "$drop_users" | head -3 | tr '\n' ', ')"
        else
            diagnosis_result="GOOD"
            status="양호"
