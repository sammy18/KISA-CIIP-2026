...
diagnose() {
    ...
    if [ -z "$found_file" ]; then
        ...
        if [ "$ftp_installed" = true ]; then
            ...
            local find_ftp=$(find /etc -name '*ftpusers*' 2>/dev/null | head -10 || echo "No ftpusers files found")
            command_result="[Command: find /etc -name '*ftpusers*']${newline}${find_ftp}"
        else
            ...
            local find_ftp=$(find /etc -name '*ftpusers*' 2>/dev/null | head -10 || echo "No ftpusers files found")
            command_result="[Command: lssrc -s ftpd]${newline}${lssrc_out}${newline}${newline}[Command: find /etc -name '*ftpusers*']${newline}${find_ftp}"
        fi
        ...
        local wc_lines=$(wc -l "$found_file" 2>/dev/null | awk '{print $1}' || echo "0")
        local cat_output=$(cat "$found_file" 2>/dev/null | head -20 || echo "File not readable")
        command_result="[Command: wc -l $found_file]${newline}Lines: ${wc_lines}${newline}${newline}[Command: cat $found_file]${newline}${cat_output}"
    else
        # 파일이 존재하고 내용이 있는 경우
        ...
        local cat_output=$(cat "$found_file" 2>/dev/null | head -30 || echo "File not readable")
        command_result="[Command: cat $found_file]${newline}${cat_output}"
        ...
        if [ ${#missing_accounts[@]} -eq 0 ]; then
            ...
            local awk_passwd=$(awk -F: '$3 >= 200 {print $1}' /etc/passwd 2>/dev/null | head -20 || echo "No users found")
            command_result="${command_result}${newline}${newline}[Command: awk -F: '\$3 >= 200' /etc/passwd]${newline}${awk_passwd}"
        fi
    fi
