...
diagnose() {
    ...
    # 4) 서비스 실행 확인
    if lssrc -s sendmail 2>/dev/null | grep -q "active" &>/dev/null; then
        mail_installed=true
    ...
    if lssrc -s postfix 2>/dev/null | grep -q "active" &>/dev/null; then
        mail_installed=true
    ...
    if lssrc -s exim4 2>/dev/null | grep -q "active" &>/dev/null; then
        mail_installed=true
    ...
    if [ "$mail_installed" = false ]; then
        ...
        local lssrc_check=$(lssrc -s sendmail postfix exim4 2>/dev/null || echo "No mail services")
        command_result="[Command: command -v sendmail postconf exim]${newline}${cmd_check}${newline}${newline}[Command: lssrc -s mail services]${newline}${lssrc_check}"
        command_executed="command -v sendmail postconf exim; lssrc -s sendmail 2>/dev/null | grep -q "active" postfix exim4"
    else
        ...
        command_executed="sendmail -d0.4 -bv < /dev/null; postconf mail_version; exim --version; lssrc -s sendmail 2>/dev/null | grep -q "active" postfix exim4"
    fi
