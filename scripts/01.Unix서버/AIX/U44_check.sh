...
diagnose() {
    ...
    if [ -f /etc/inetd.conf ]; then
        local tftp_entry=$(grep "^tftp" /etc/inetd.conf 2>/dev/null | grep -v "^#" || echo "")
        if [ -n "$tftp_entry" ]; then
            services_running=true
    ...
    if [ -f /etc/inetd.conf ]; then
        local talk_services=$(grep -E "^talk|^ntalk" /etc/inetd.conf 2>/dev/null | grep -v "^#" || echo "")
    ...
    if [ "$services_running" = false ]; then
        ...
        local lssrc_out=$(lssrc -s tftp 2>/dev/null || echo "Service not found")
        local ss_out=$(ss -tuln | grep -E ':69|:517|:518' 2>/dev/null || echo "Ports not listening")
        command_result="[Command: lssrc -s tftp]${newline}${lssrc_out}${newline}${newline}[Command: ss -tuln | grep tftp/talk]${newline}${ss_out}"
        command_executed="lssrc -s tftp 2>/dev/null | grep -q "active" talk ntalk; ss -tuln | grep -E ':69|:517|:518'"
    else
        diagnosis_result="VULNERABLE"
        status="취약"
        inspection_summary="불필요한 서비스 활성화됨: ${running_services[*]}"
        command_result="${service_info}"
        command_executed="lssrc -s tftp 2>/dev/null | grep -q "active" talk ntalk; grep -E '^talk|^ntalk' /etc/inetd.conf"
    fi
