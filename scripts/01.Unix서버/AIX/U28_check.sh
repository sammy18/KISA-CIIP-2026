...
diagnose() {
    ...
    if [ -f "/etc/hosts.allow" ]; then
        ...
        local perms=$(ls -ld "/etc/hosts.allow" 2>/dev/null | awk '{print $1}' | cut -c2-10 | sed 's/rwx/7/g; s/rw-/6/g; s/r-x/5/g; s/r--/4/g; s/-wx/3/g; s/-w-/2/g; s/--x/1/g; s/---/0/g')
        local owner=$(ls -ld "/etc/hosts.allow" 2>/dev/null | awk '{print $3":"$4}')
    ...
    if [ -f "/etc/hosts.deny" ]; then
        ...
        local perms=$(ls -ld "/etc/hosts.deny" 2>/dev/null | awk '{print $1}' | cut -c2-10 | sed 's/rwx/7/g; s/rw-/6/g; s/r-x/5/g; s/r--/4/g; s/-wx/3/g; s/-w-/2/g; s/--x/1/g; s/---/0/g')
        local owner=$(ls -ld "/etc/hosts.deny" 2>/dev/null | awk '{print $3":"$4}')
    ...
    if [ "$hosts_allow_exists" -eq 0 ] && [ "$hosts_deny_exists" -eq 0 ]; then
        ...
        local ls_output=$(ls -l /etc/hosts.allow /etc/hosts.deny 2>/dev/null || echo "Files not found")
        command_result="[Command: ls -l /etc/hosts.allow /etc/hosts.deny]${newline}${ls_output}"
        ...
        local grep_allow=$(grep -v '^#' /etc/hosts.allow | grep -v '^$' 2>/dev/null | wc -l)
        local ls_output=$(ls -l /etc/hosts.allow /etc/hosts.deny 2>/dev/null || echo "Files not found")
        command_result="[Command: ls -l hosts files]${newline}${ls_output}${newline}${newline}[Command: grep -v '^#' /etc/hosts.allow]${newline}Non-comment lines: ${grep_allow}"
        ...
        local ls_output=$(ls -l /etc/hosts.deny 2>/dev/null || echo "File not found")
        command_result="[Command: ls -l /etc/hosts.deny]${newline}${ls_output}"
        ...
        local cat_allow=$(cat /etc/hosts.allow 2>/dev/null | head -10 || echo "File not readable")
        local cat_deny=$(cat /etc/hosts.deny 2>/dev/null | head -10 || echo "File not readable")
        command_result="[Command: cat /etc/hosts.allow]${newline}${cat_allow}${newline}${newline}[Command: cat /etc/hosts.deny]${newline}${cat_deny}"
