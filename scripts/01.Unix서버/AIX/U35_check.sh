...
diagnose() {
    ...
    if command -v vsftpd &>/dev/null || command -v proftpd &>/dev/null || command -v pure-ftpd &>/dev/null; then
        ...
        if [ -f /etc/vsftpd.conf ]; then
            ...
            ftp_config="${ftp_config}$(grep -iE '^anonymous_enable|^anon_enable' /etc/vsftpd.conf 2>/dev/null || echo 'vsftpd: 설정 없음')\\n"
        fi
        ...
        if [ -f /etc/proftpd/proftpd.conf ]; then
            ...
            ftp_config="${ftp_config}$(grep -iA5 '<Anonymous' /etc/proftpd/proftpd.conf 2>/dev/null || echo 'proftpd: 설정 없음')\\n"
        fi
        ...
        if [ -f /etc/pure-ftpd/conf/NoAnonymous ]; then
            ...
            ftp_config="${ftp_config}Pure-FTPD: NoAnonymous=${no_anonymous}\\n"
        fi
    fi

    # 2) FTP 서비스 실행 확인
    if lssrc -s vsftpd 2>/dev/null | grep -q "active" &>/dev/null || lssrc -s proftpd 2>/dev/null | grep -q "active" &>/dev/null || lssrc -s pure-ftpd 2>/dev/null | grep -q "active" &>/dev/null; then
        ftp_installed=true
        ftp_config="${ftp_config}FTP 서비스 실행 중\\n"
    fi
    ...
    if command -v ss &>/dev/null; then
        ...
        if [ -n "$ftp_port" ]; then
            ftp_installed=true
            ftp_config="${ftp_config}FTP 포트 21 활성화\\n"
        fi
    fi
    ...
    if [ "$ftp_installed" = false ]; then
        ...
        local cmd_check=$(command -v vsftpd proftpd pure-ftpd 2>&1 || echo "No FTP commands found")
        local lssrc_check=$(lssrc -s vsftpd proftpd pure-ftpd 2>/dev/null || echo "No FTP services active")
        command_result="[Command: command -v vsftpd proftpd pure-ftpd]${newline}${cmd_check}${newline}${newline}[Command: lssrc -s ftp services]${newline}${lssrc_check}"
        command_executed="command -v vsftpd proftpd pure-ftpd; lssrc -s vsftpd 2>/dev/null | grep -q "active" proftpd pure-ftpd"
