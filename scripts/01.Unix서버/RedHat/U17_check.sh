...
diagnose() {
    ...
    while IFS= read -r file; do
        if [ -n "$file" ]; then
            ...
            local expected_suid_patterns="^(ping|ping6|traceroute|traceroute6|sudo|passwd|su|gpasswd|chsh|chfn|newgrp|umount|mount|pkexec|at|fusermount|Xorg|wbem)"
    ...
    done < <(find / -perm -2000 -type f 2>/dev/null | head -50)
