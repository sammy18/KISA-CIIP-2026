...
diagnose() {
    ...
    if command -v apt >/dev/null 2>&1; then
        ...
        # RHEL/CentOS의 경우 (yum check-update는 업데이트 없으면 exit code 100 반환)
        local update_list=$(yum check-update 2>/dev/null || true)

        # 명령어 실패 시에도 계속 진행
        update_list=$(echo "$update_list" | grep -v "^$" | grep -v "Loaded plugins" | tail -n +2 || true)

        if [ -n "$update_list" ]; then
            package_updates=$(echo "$update_list" | wc -l)
            security_updates=$(echo "$update_list" | grep -i "security" | wc -l || echo "0")
        else
            package_updates=0
            security_updates=0
        fi
        ...
        raw_output=$(echo "=== Kernel Version ===" && uname -r && echo -e "\n=== Yum Check Update ===" && yum check-update 2>/dev/null | head -20 || echo "No updates available")

        # 마지막 yum history 확인
        local last_yum=$(yum history list 2>/dev/null | head -5 | tail -1 | awk '{print $1" "$3" "$4" "$5" "$6" "$7}' || echo "")
    ...
    if [ "$security_updates" -gt 0 ]; then
        ...
        command_executed="uname -r; yum check-update 2>/dev/null | head -20"
    elif [ "$package_updates" -gt 10 ]; then
        diagnosis_result="VULNERABLE"
        status="취약"
        inspection_summary="일반 업데이트 ${package_updates}개 미적용 (시스템 업데이트 권장): ${details}"
        command_result="${raw_output}"
        command_executed="uname -r; yum check-update 2>/dev/null | wc -l"
    else
