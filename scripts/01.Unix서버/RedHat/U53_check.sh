...
diagnose() {
    ...
    if [ "$ftp_banner_issue" = true ]; then
        ...
        if [ "$ftp_installed" = true ]; then
            ...
            local raw_banner_check=$(grep -E 'ftpd_banner|ServerIdent' /etc/{vsftpd,vsftpd/vsftpd,proftpd/proftpd}.conf 2>/dev/null || echo "No banner settings found")
            command_result="[Command: grep -E 'ftpd_banner|ServerIdent' /etc/{vsftpd,vsftpd/vsftpd,proftpd/proftpd}.conf 2>/dev/null]${newline}${raw_banner_check}"
            command_executed="grep -E 'ftpd_banner|ServerIdent' /etc/{vsftpd,vsftpd/vsftpd,proftpd/proftpd}.conf 2>/dev/null"
        else
            diagnosis_result="GOOD"
            status="양호"
            inspection_summary="FTP 서비스가 설치되지 않음"
            local raw_ftp_check=$(ls /etc/{vsftpd.conf,proftpd/proftpd.conf} 2>&1)
            command_result="[Command: ls /etc/{vsftpd.conf,proftpd/proftpd.conf} 2>/dev/null]${newline}${raw_ftp_check}"
