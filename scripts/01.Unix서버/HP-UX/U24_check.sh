...
diagnose() {
    ...
    # /etc/passwd에서 일반 사용자(UID >= 100)의 홈 디렉터리 확인 (HP-UX: 시스템 UID < 100) - 실제 명령어 결과 저장
    local raw_output=""
    while IFS= read -r user_line; do
        ...
        # 홈 디렉터리가 존재하고, UID가 100 이상인 일반 사용자 확인
    ...
    if [ "$vulnerable_count" -eq 0 ]; then
        if [ "$checked_count" -eq 0 ]; then
            ...
            command_executed="awk -F: '\$3 >= 100 {print \$1, \$6}' /etc/passwd | while read user home; do find \"\$home\" -maxdepth 1 -name '.*' -type f 2>/dev/null; done | xargs perl -e 'for (@ARGV) {@s=lstat; printf \"%04o %s %s\\n\", \$s[2]&07777, getpwuid(\$s[4]), $_}' 2>/dev/null"
        else
            diagnosis_result="GOOD"
            status="양호"
