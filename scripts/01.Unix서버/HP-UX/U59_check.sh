...
diagnose() {
    ...
    if [ "$snmpd_installed" = false ]; then
        ...
        command_executed="/sbin/init.d/snmpd status 2>/dev/null | grep -q "running" 2>/dev/null"
    else
        ...
        if [ -f "$snmp_conf" ]; then
            ...
            if grep -qi "createUser\|rouser\|rwuser" "$snmp_conf" 2>/dev/null; then
                ...
                if grep -qi "com2sec" "$snmp_conf" 2>/dev/null; then
                    ...
                    local snmp_versions=$(grep -E 'com2sec|createUser|rouser' /etc/snmp/snmpd.conf 2>/dev/null | head -5)
                    command_result="SNMP v1/v2c+v3 detected\n${snmp_versions}"
                    ...
                    local snmp_v3=$(grep -E 'createUser|rouser|rwuser' /etc/snmp/snmpd.conf 2>/dev/null | head -3)
                    command_result="SNMP v3 detected\n${snmp_v3}"
                ...
                local snmp_v1=$(grep com2sec /etc/snmp/snmpd.conf 2>/dev/null | head -3)
                command_result="SNMP v1/v2c detected\n${snmp_v1}"
                ...
                local snmp_unknown=$(cat /etc/snmp/snmpd.conf 2>/dev/null | head -10)
                command_result="SNMP version: unknown\n${snmp_unknown}"
            ...
            local no_conf=$(ls /etc/snmp/*.conf 2>/dev/null || echo "No SNMP config files")
