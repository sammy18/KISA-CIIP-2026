...
diagnose() {
    ...
    if [ -z "$target_file" ] || [ ! -f "$target_file" ]; then
        ...
        local ls_output=$(ls -l /etc/rsyslog.conf /etc/syslog.conf 2>/dev/null || echo "File not found")
        command_result="${ls_output}"
        command_executed="ls -l /etc/rsyslog.conf /etc/syslog.conf 2>&1"
    else
        # 파일 권한 확인 (HP-UX: stat -c 대신 perl 사용)
        ...
        if [ "$is_secure" = true ]; then
            ...
            command_executed="perl -e 'printf \"%04o\\n\", (stat)[2] & 07777' $target_file; perl -e '@stat=lstat; print getpwuid($stat[4]).\":\".getgrgid($stat[5])' $target_file"
        else
