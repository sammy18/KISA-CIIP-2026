...
diagnose() {
    ...
    # 3) systemd 서비스 확인 (HP-UX: /sbin/init.d 사용)
    service_status="${service_status}${newline}서비스 확인:${newline}"
    for svc in "${vulnerable_services[@]}"; do
        if [ -f "/sbin/init.d/${svc}" ] || [ -f "/sbin/init.d/${svc}.service" ]; then
            local active=$(/sbin/init.d/${svc} status 2>/dev/null | grep -q "running" 2>/dev/null && echo "active" || echo "inactive")
            service_status="${service_status}  ${svc}: ${active}${newline}"

            if [ "$active" = "active" ]; then
                is_secure=false
                enabled_services+=("service: ${svc}")
            fi
        fi
    done || true
    ...
    if [ "$is_secure" = true ]; then
        ...
        command_result="${service_status}"
        command_executed="grep -iE '^echo|^chargen|^daytime' /etc/inetd.conf 2>/dev/null; /sbin/init.d/echo status 2>/dev/null | grep -q "running" chargen daytime 2>/dev/null"
    fi
