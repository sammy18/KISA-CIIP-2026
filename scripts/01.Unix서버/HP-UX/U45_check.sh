...
diagnose() {
    ...
    if command -v sendmail &>/dev/null; then
        ...
        mail_info="${mail_info}Sendmail: ${mail_version}\\n"
    else
    ...
    if command -v postconf &>/dev/null; then
        ...
        mail_info="${mail_info}Postfix: ${mail_version}\\n"
    else
    ...
    if command -v exim &>/dev/null; then
        ...
        mail_info="${mail_info}Exim: ${mail_version}\\n"
    else
        exim_cmd_output="exim 명령어 없음"
    fi

    # 4) 서비스 실행 확인
    if /sbin/init.d/sendmail status 2>/dev/null | grep -q "running" &>/dev/null; then
        mail_installed=true
        sendmail_svc_output="active"
        mail_info="${mail_info}Sendmail 서비스 실행 중\\n"
    else
        sendmail_svc_output="inactive or not installed"
    fi

    if /sbin/init.d/postfix status 2>/dev/null | grep -q "running" &>/dev/null; then
        mail_installed=true
        postfix_svc_output="active"
        mail_info="${mail_info}Postfix 서비스 실행 중\\n"
    else
        postfix_svc_output="inactive or not installed"
    fi

    if /sbin/init.d/exim4 status 2>/dev/null | grep -q "running" &>/dev/null; then
        mail_installed=true
        exim_svc_output="active"
        mail_info="${mail_info}Exim 서비스 실행 중\\n"
    else
    ...
    if [ "$mail_installed" = false ]; then
        ...
        local mail_check=$(command -v sendmail 2>/dev/null; command -v postconf 2>/dev/null; command -v exim 2>/dev/null; /sbin/init.d/sendmail status 2>/dev/null | head -2; /sbin/init.d/postfix status 2>/dev/null | head -2 || echo "No mail services found")
        command_result="${mail_check}"
        command_executed="command -v sendmail postconf exim; /sbin/init.d/sendmail status 2>/dev/null | grep -q "running" postfix exim4"
    else
        ...
        command_executed="sendmail -d0.4 -bv < /dev/null; postconf mail_version; exim --version; /sbin/init.d/sendmail status 2>/dev/null | grep -q "running" postfix exim4"
    fi
