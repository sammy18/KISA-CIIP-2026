...
diagnose() {
    ...
    # 1) Solaris SMF 서비스 실행 여부 확인
    if svcs telnet 2>/dev/null | grep -q "online" || \
       svcs network/telnet 2>/dev/null | grep -q "online"; then
        telnet_running=true
        telnet_details="SMF 서비스 상태: online"
    fi

    # 2) Telnet 서비스 실행 여부 확인 (inetd.conf)
    ...
    # 3) Telnet 포트Listening 확인 (Solaris는 netstat 사용)
    if command -v netstat >/dev/null 2>&1; then
        if netstat -an 2>/dev/null | grep -q "\.23 "; then
            telnet_running=true
            telnet_details="${telnet_details}, 포트 23 listening"
        fi
    fi

    # 4) Telnet 패키지 설치 여부 확인 (Solaris pkginfo/pkg)
    local telnet_installed=""
    if command -v pkginfo &>/dev/null; then
        if pkginfo 2>/dev/null | grep -q "SUNWtnet"; then
            telnet_installed="SUNWtnet 패키지 설치됨"
        fi
    elif command -v pkg &>/dev/null; then
        if pkg list 2>/dev/null | grep -q "telnet"; then
            telnet_installed="Telnet IPS 패키지 설치됨"
        fi
    fi
    ...
    if [ "$telnet_running" = true ]; then
        ...
        command_executed="svcs telnet network/telnet 2>/dev/null; netstat -an | grep '\.23 '; grep '^telnet' /etc/inetd.conf 2>/dev/null"
    else
        diagnosis_result="GOOD"
        status="양호"
        if [ -n "$telnet_installed" ]; then
            inspection_summary="Telnet 서비스는 비활성화되어 있으나 패키지는 설치됨 (${telnet_installed})"
            local telnet_check=$(svcs telnet network/telnet 2>/dev/null || echo "Service not found")
            command_result="[Command: svcs telnet network/telnet]\${newline}${telnet_check}"
        else
            inspection_summary="Telnet 서비스가 설치되어 있지 않거나 비활성화됨"
            local telnet_check=$(svcs telnet 2>/dev/null || echo "Service not found")
            command_result="[Command: svcs telnet]\${newline}${telnet_check}"
        fi
        command_executed="svcs telnet 2>/dev/null; netstat -an | grep '\.23 '"
    fi
