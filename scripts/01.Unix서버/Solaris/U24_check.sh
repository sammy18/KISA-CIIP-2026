...
diagnose() {
    ...
    # /etc/passwd에서 일반 사용자(UID >= 100)의 홈 디렉터리 확인 (Solaris: UID < 100은 시스템 계정) - 실제 명령어 결과 저장
    local raw_output=""
    while IFS= read -r user_line; do
        ...
        # 홈 디렉터리가 존재하고, UID가 100 이상인 일반 사용자 확인 (Solaris)
