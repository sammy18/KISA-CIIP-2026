...
diagnose() {
    ...
    if [ -z "$target_file" ] || [ ! -f "$target_file" ]; then
        ...
        local syslog_check=$(ls -l /etc/rsyslog.conf /etc/syslog.conf 2>&1 || echo "Files not found")
        command_result="[Command: ls -l /etc/rsyslog.conf /etc/syslog.conf]${newline}${syslog_check}"
        command_executed="ls -l /etc/rsyslog.conf /etc/syslog.conf 2>&1"
    else
        # 파일 권한 확인 (Solaris: perl stat 사용)
        ...
        if [ "$is_secure" = true ]; then
            ...
            command_executed="perl -e 'printf \"%04o\n\", (stat)[2] & 07777' $target_file; ls -l $target_file"
        else
