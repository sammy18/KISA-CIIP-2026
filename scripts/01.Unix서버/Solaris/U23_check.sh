...
diagnose() {
    ...
    # 추가: 중요 설정 파일의 권한 확인 (Solaris: /etc/gshadow 없음) - 실제 명령어 결과 저장
    local important_files="/etc/passwd /etc/shadow /etc/group /etc/hosts /etc/services /etc/inetd.conf /etc/syslog.conf"
    ...
    if [ "$ww_count" -eq 0 ] && [ "$important_count" -eq 0 ]; then
        ...
        command_executed="find /etc /usr/bin /usr/sbin -perm -2 -type f 2>/dev/null; perl -e 'for $f (@ARGV) { printf \"%04o %s\n\", (stat($f))[2] & 07777, $f if -f $f; }' /etc/passwd /etc/shadow /etc/group"
    else
