...
diagnose() {
    ...
    if [ "$services_running" = false ]; then
        ...
        command_result="$(systemctl is-active tftp 2>&1 || echo 'inactive')${newline}$(systemctl is-active talk 2>&1 || echo 'inactive')${newline}$(systemctl is-active ntalk 2>&1 || echo 'inactive')${newline}${newline}[Port Check]${newline}$(ss -tuln 2>/dev/null | grep -E ':69|:517|:518' || echo 'No tftp/talk services listening')"
