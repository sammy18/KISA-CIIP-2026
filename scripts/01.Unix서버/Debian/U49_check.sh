...
diagnose() {
    ...
    if [ "$dns_installed" = false ]; then
        ...
        command_result="[DNS Service Check]${newline}$(command -v named 2>&1 || echo 'named not found')${newline}$(systemctl is-active named 2>&1 || echo 'inactive')${newline}$(systemctl is-active bind9 2>&1 || echo 'inactive')${newline}${newline}[Port 53 Check]${newline}$(ss -tuln 2>/dev/null | grep ':53' || echo 'No DNS service listening on port 53')"
