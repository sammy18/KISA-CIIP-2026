...
diagnose() {
    ...
    if systemctl is-active rsyslog >/dev/null 2>&1; then
        ...
        raw_output=$(systemctl status rsyslog 2>/dev/null || echo "rsyslog not found"; systemctl status syslog-ng 2>/dev/null || echo "syslog-ng not found")
    ...
    if [ ! -f "$config_file" ]; then
        ...
        raw_output=$(ls -l "$config_file" 2>/dev/null || echo "File not found")
        command_result="${raw_output}"
        command_executed="ls -l ${config_file}"
    else
        # Capture raw syslog config
        raw_output=$(grep -v "^#" "$config_file" 2>/dev/null | grep -v "^[[:space:]]*$" | head -20 || echo "Empty config")
