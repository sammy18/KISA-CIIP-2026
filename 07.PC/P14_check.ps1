...
$purpose = '사용자가 인터넷(이동식 저장 매체 포함)을 통해 파일을 다운로드하거나 다운로드 받은 파일을 실행할 경우백신프로그램이악성코드감염을실시간으로점검하고있는지확인하기위함'
$threat = '백신 프로그램의 실시간 감시기능이 적용되어 있지 않을 경우, 악성코드에 대해 실시간 감지가 이루어지지 않아 시스템 사용자가 인터넷(이동식 저장 매체 포함)을 통한 파일 다운로드나 실행 시 악성코드가감염될위험이존재함'
$criteria_good = '설치된백신의실시간감시기능이활성화된경우'
$criteria_bad = '백신이설치되어있지않거나실시간감시기능이비활성화된경우'
$remediation = '바이러스백신실시간감시기능설정'
