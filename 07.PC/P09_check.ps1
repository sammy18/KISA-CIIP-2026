...
# 2. lib를 통한 결과 저장
$purpose = '브라우저 사용 시 생성되는 임시 인터넷 파일 삭제를 통하여 웹 양식에 입력한 정보(예: 이름 및 주소), 자동로그인을위한웹사이트비밀번호정보등을삭제하여개인정보의보안을향상하기위함'
$threat = '임시 인터넷 파일 폴더 내용을 삭제하지 않을 경우, 다른 계정에 저장된 임시 인터넷 파일 폴더를 통해 이메일주소,웹사이트접근기록등의개인정보를획득할수있는위험이존재함'
$criteria_good = '"브라우저를닫을때임시인터넷파일폴더비우기''설정이''사용''으로설정된경우'
$criteria_bad = '"브라우저를닫을때임시인터넷파일폴더비우기''설정이''미사용''으로설정된경우'
