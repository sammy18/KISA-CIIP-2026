...
# 2. lib를 통한 결과 저장
$purpose = '사용하지 않는 불필요한 공유 폴더를 해제하거나 불가피하게 사용하고 있는 공유 폴더의 경우 비밀번호를 설정하는 등의 조치를 통해 인가된 사용자만 접근할 수 있게 함으로써 무분별한 접근을 제한하기위함'
$threat = 'Ÿ 시스템기본공유폴더의경우기본드라이브를개방해놓고사용하는것과같은위험이존재함(예시: 실행창->\\192.168.16.xxx\C$로C드라이브접근가능) Ÿ 접근 권한이 Everyone으로 설정된 공유 폴더는 정보 유출 및 악성코드 유포의 접점이 될 수 있는 위험이존재함'
$criteria_good = '불필요한공유폴더가존재하지않거나공유폴더에접근권한및비밀번호가설정된경우'
$criteria_bad = '불필요한공유폴더가존재하거나접근권한및비밀번호설정없이공유폴더가사용된경우'
$remediation = 'Ÿ 공유폴더불필요시삭제 Ÿ 공유폴더필요하면적절한접근권한부여및비밀번호설정 Ÿ 조치후''AutoShareWks''값변경으로자동공유방지'
