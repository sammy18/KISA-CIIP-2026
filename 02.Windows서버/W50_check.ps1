...
# 2. lib를 통한 결과 저장
$purpose = "해당 정책을 비활성화함으로써 로그 용량 초과 등의 이유로 이벤트를 기록할 수 없는 경우, 해당 정책으로인해시스템이비정상적으로종료되는것을방지하기위함"
$threat = "해당 정책이 활성화되어 있는 경우 악의적인 목적으로 시스템 종료를 유발하여 서비스 거부 공격에 악용될수있으며,비정상적인시스템종료로인해시스템및데이터에손상을입힐위험이존재함"
$criteria_good = "'보안감사를로그할수없는경우즉시시스템종료'정책이'사용안함'으로되어있는경우"
$criteria_bad = "'보안감사를로그할수없는경우즉시시스템종료'정책이'사용'으로되어있는경우"
$remediation = "'보안감사를로그할수없는경우즉시시스템종료'정책을'사용안함'으로설정"
