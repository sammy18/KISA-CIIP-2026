...
# 2. lib를 통한 결과 저장
$purpose = "이 설정은 Kerberos가 클라이언트 시계의 시간과 서버 시계의 시간 사이에서 허용되는 최대 시간 차이(분)를결정하는동시에두시계의동기를고려하여,재전송공격을방지하기위함"
$threat = "Replay Attack 이란 프로토콜 상 메시지를 복사한 후 재전송함으로써 승인된 사용자로 오인하게 만들어내부침입및정보유출위험이존재함"
$criteria_good = "컴퓨터시계동기화최대허용오차값이5분이하인경우"
$criteria_bad = "컴퓨터시계동기화최대허용오차값이5분초과인경우"
$remediation = "Kerberos사용시컴퓨터시계동기화최대허용오차값5분이하로설정"
