...
# 2. lib를 통한 결과 저장
$purpose = "사용자가 일정 시간 동안 아무런 작업을 수행하지 않으면 자동으로 로그오프 되거나 워크스테이션이 잠기도록설정하여,유휴시간내불법적인시스템접근을차단하기위함"
$threat = "화면보호기설정을하지않으면사용자가자리를비운사이에임의의사용자가해당시스템에접근하여 중요정보를유출하거나,악의적인행위를통해시스템운영에악영향을미칠위험이존재함"
$criteria_good = "화면보호기를설정하고대기시간이10분이하의값으로설정되어있으며, 화면보호기해제를 위한암호를사용하는경우"
$criteria_bad = "화면보호기가설정되지않거나대기시간이10분초과또는암호사용이비활성화된경우"
$remediation = "화면보호기사용,대기시간10분이하,해제를위한암호사용"
