...
# [점검 항목 상세]
# @ID          : All
# @Category    : Windows Server (윈도우 서버)
# @Platform    : Windows Server
# @Severity    : 중
# @Title       : 모든 Windows 서버 점검 스크립트 실행
# @Description : Windows Server 모든 점검 항목을 실행하는 스크립트 (PowerShell 형식)
...
# 진단 항목 목록 (W-01~W-64)
$DIAGNOSIS_ITEMS = 1..64 | ForEach-Object { "W-{0:D2}" -f $_ }
...
function Run-SingleCheck {
    ...
    try {
        ...
        # 환경 변수 설정 (자식 스크립트에서 Test-RunallMode로 감지)
        $env:WS_RUNALL_MODE = "1"
        try {
            $output = & $scriptFile 2>&1 | Out-String
        } finally {
            # 항상 환경 변수 정리
            Remove-Item env:WS_RUNALL_MODE -ErrorAction SilentlyContinue
        }
    ...
    # 디버깅: 임시 파일 보존 (수동 확인용)
    if (-not $jsonOutput) {
        Write-Warning "JSON 추출 실패 - 임시 파일 보존: $tmpOutput"
    } else {
        Remove-Item $tmpOutput -Force -ErrorAction SilentlyContinue
    }
