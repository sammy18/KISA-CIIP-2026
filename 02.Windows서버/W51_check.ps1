...
$threat = "Windows에서는 익명의 사용자가 도메인 계정(사용자, 컴퓨터 및 그룹)과 네트워크 공유 이름의 열거 작업을 수행할 수있으므로SAM(보안 계정 관리자) 계정과 공유의 익명 열거가 허용될 경우 악의적인 사용자가 계정 이름 목록을 확인하고 이 정보를 사용하여 암호를 추측하거나 사회 공학적 공격기법을 수행할위험이존재함"
$criteria_good = "'SAM계정과공유의익명열거허용안함'이'사용'으로설정된경우"
$criteria_bad = "'SAM계정과공유의익명열거허용안함'이'사용안함'으로설정된경우"
$remediation = "레지스트리값또는,로컬보안정책설정"
