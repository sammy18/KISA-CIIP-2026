...
# 2. lib를 통한 결과 저장
$purpose = "계정잠금임계값을설정하여공격자의자유로운자동화암호유추공격을차단하기위함"
$threat = "공격자는 시스템의 계정 잠금 임계값이 설정되지 않는 경우, 자동화된 방법을 이용하여 모든 사용자 계정에대해암호조합공격을자유롭게시도할수있으므로사용자계정정보의노출위험이존재함"
$criteria_good = "계정잠금임계값이5이하의값으로설정된경우"
$criteria_bad = "계정잠금임계값이5초과의값으로설정된경우"
$remediation = "계정잠금임계값을5이하의값으로설정"
